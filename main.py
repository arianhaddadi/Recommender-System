...
#     personality_matrix.to_csv(Address.PERSONALITY_DIFFERENCES_PEARSON)
...
def prepareSummary(predictions_dict, neighbours, test_set_dict, train_set_dict):
    trust_scores = pd.read_csv(Address.TRUST_SCORE)
    trust_scores.columns.values[0] = "username"
    trust_scores = trust_scores.set_index("username")
...
def calFeaturesAndPredict(train_set, test_set, command):
    ...
    train_biases(test_set_dict, reviews, neighbours, biases, num_of_epochs=150, lambda_value=0.2, learning_rate=0.1)

    predictions_res = predict(test_set_dict, reviews, neighbours, biases, predictions_dict=predictions_dict)
